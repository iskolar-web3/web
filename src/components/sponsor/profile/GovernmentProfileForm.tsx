...
import {
	AgencyType,
	updateGovernmentSponsorRequestSchema,
 ...
 */
const GovernmentSponsorProfileForm = forwardRef<
	HTMLFormElement,
	GovernmentSponsorProfileFormProps
					...
					<div className="min-h-[40px] px-4 bg-[#F9FAFB] border border-border rounded-sm flex items-center">
						<p className="text-xs md:text-sm text-primary">
							{[
								...
								{
									value: AgencyType.LocalGovernmentUnit,
							...
							].find((opt) => opt.value === profile.agencyType.code)?.label ||
								profile.agencyType.name ||
								"—"}
						</p>
					</div>
				</div>
