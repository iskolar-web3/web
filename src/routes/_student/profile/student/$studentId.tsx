...
import { User, Mail, Phone, Calendar, Users, Edit, AlertCircle, X, Save } from 'lucide-react';
 ...
 * Displays comprehensive student information including personal details
 */
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        if (result.success && result.profile) {
          ...
          if (result.profile.role === 'student') {
            ...
            if (USE_MOCK_DATA) {
              ...
              setError('Profile type mismatch');
            }
          }
        } else {
        ...
        // Use mock data in development, show error in production
        if (USE_MOCK_DATA) {
          logger.info('Using mock student data as fallback in development');
          setProfile(mockStudentUser);
        } else {
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            <InfoField 
              label="Full Name" 
              value={isEditing && editedProfile ? editedProfile.full_name : profile.full_name}
              isEditing={isEditing}
              onChange={(value) => handleFieldChange('full_name', value)}
            />
            <SelectField 
              label="Gender" 
              value={isEditing && editedProfile ? editedProfile.gender : profile.gender}
              options={[
            ...
            <DateField
              label="Date of Birth"
              value={isEditing && editedProfile ? editedProfile.date_of_birth : profile.date_of_birth}
              isEditing={isEditing}
 ...
 */
function InfoField({ label, value, icon, isEditing = false, onChange, type = 'text' }: InfoFieldProps) {
  return (
    <div>
